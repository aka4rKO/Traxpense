...
class MasterViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.categoryLineView.backgroundColor = Utilities.chooseColor(category.color!)
        return cell
    }
    ...
    func deleteAction (at indexPath: IndexPath) -> UIContextualAction {
        let action = UIContextualAction(style: .normal, title: "Delete") { (action, view, completion) in
            Utilities.showConfirmationAlert(title: "Are you sure?", message: "Deleting " + self.fetchedResultsController.object(at: indexPath).name!, yesAction: {() in
                let context = self.fetchedResultsController.managedObjectContext
    ...
    @IBAction func sort(_ sender: Any) {
        do {
            ...
            if(sortingMethod == "clickCount"){
                sortBtn.image = UIImage(systemName: "textformat.123")
                sortingMethod = "name"
                isAscending = true
            } else {
                sortBtn.image = UIImage(systemName: "textformat.abc")
